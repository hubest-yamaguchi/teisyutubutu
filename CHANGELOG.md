...
## 2026-09-11（続き6）— 運転免許証をマイナンバーと同じ表裏2枚アップロード方式に統合

- 「運転免許証（表面）」「運転免許証（裏面）」の別書類2件方式をやめ、マイナンバー確認書類と同じく
  「運転免許証」1項目で表裏2枚アップロードする方式に統合した(licenseFrontに一本化、licenseBackは廃止)
- jinjer連携も表裏それぞれ別のカスタム項目ID(表=18、裏=19)に送信できるよう対応
  (company_document_config.JinjerCustomItemId2、submissions.JinjerSentStorageKey2を追加。
  書類マスタ編集画面にも2枚目用カスタム項目IDの入力欄と「表裏2枚アップロード可」チェックボックスを追加)
- 本番のcompany_document_configには実データがまだ無かったため(表裏とも未提出の社員1名のみ)、
  データ移行は不要だったが、本番データの直接更新は別途必要(本記録の下に対応内容を追記)

## 2026-09-11（続き5）— マイナンバー確認書類の表裏2枚提出・承認取り消し機能を追加

- マイナンバー確認書類を「表面」「裏面」の2枚アップロードできるようにした。書類自体は1項目のまま、
  承認・差し戻しなどのステータスは共通で、ファイルの実体だけ2つ持てるようにした(`submissions`テーブルに
  `StorageKey2`/`MimeType2`を追加。書類マスタに`DualFile`列を追加し、対応する書類にチェックを入れると
  この動作になる。運転免許証表裏のような別書類2件方式とは異なり、1つのメニュー項目で完結する)
- 管理画面: 承認済みの書類を誤って承認してしまった場合に「確認中」へ戻せる「承認を取り消す」ボタンを追加
  (原本必要な書類は既存の「原本受領を取消」ボタンで「原本提出待ち」に戻せるため対象外)

## 2026-09-11（続き4）— ⚠️マークの廃止・書類マスタの本番データとコードの不一致を修正

- 原本提出待ちの表示から⚠️マークを廃止し、「承認済」ピル+赤字「原本提出待ち」のみに統一
- 重要: `company_document_config`テーブルがすでに行を持っている場合、`src/model.ts`の`DOC_TYPES`は
  フォールバックとして一切使われない(テーブルが完全に空の時だけ使われる)。そのため、このセッションで
  model.tsに加えていた説明文・見本画像・⚠️ラベル等の変更は、本番のテーブルにはずっと反映されていなかった
  （画面に⚠️が残っていたのはこれが原因。本番DBのLabel/Description列に文字列としてそのまま残っていた）
- 本番の`company_document_config`を直接確認し、⚠️の文字列をLabel/Descriptionから削除（データ修正、コードのデプロイ不要）
- 本番の実データ(マニュアルPDFをもとにした本来の内容)の方が`model.ts`の内容より正確だったため、
  `model.ts`の`DOC_TYPES`を本番の実データに合わせて同期した(万一テーブルが空になった場合のフォールバックが
  正しい内容になるように。今回の同期で本番の動作自体は変わらない)
